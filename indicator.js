...
class AwsIndicator extends PanelMenu.Button {
    ...
    _sessionChanged(monitor, fileMonitored, otherFile, eventType) {
        ...
        // Ignore events outside the sessions directory tree (from parent dir monitors)
        const changedPath = fileMonitored.get_path();
        if (!changedPath.startsWith(this._sessions_path) &&
            !this._sessions_path.startsWith(changedPath)) {
            return;
        }
